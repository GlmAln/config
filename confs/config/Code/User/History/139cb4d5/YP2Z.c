...
void set_play(Game* game)
{
    ...
    game->disc.event = g_timeout_add(4, on_move_disc, game);
}
...
gboolean paddle_move_up1(gpointer user_data)
{

    // Gets the 'Game' structur
    Game *game = user_data;

    // Max cordinates for the paddle in y axes
    gint y_max = gtk_widget_get_allocated_height(GTK_WIDGET(game->ui.area))
        - game->p1.rect.height;

    GdkRectangle old = game->p1.rect;

    // Works out the new position of the paddle.
    player.rect.y = CLAMP(game->p1.rect.y - game->p1.step, 0, y_max);
...
gboolean paddle_move_down1(gpointer user_data)
{
...
gboolean paddle_move_up2(gpointer user_data)
{
    ...
    // Works out the new position of the paddle.
    player.rect.y = CLAMP(game->p2.rect.y - game->p2.step, 0, y_max);
...
gboolean paddle_move_down2(gpointer user_data)
{
...
gboolean on_key_press(GtkWidget *widget, GdkEventKey *event, gpointer user_data)
{
    ...
    {
        game->p1.event = g_timeout_add(4, paddle_move_up1, game);
        return TRUE;
    }
    ...
    {
        game->p1.event = g_timeout_add(4, paddle_move_down1, game);
        return TRUE;
    }
    ...
    {
        game->p1.event = g_timeout_add(4, paddle_move_up2, game);
        return TRUE;
    }
    ...
    {
        game->p1.event = g_timeout_add(4, paddle_move_down2, game);
        return TRUE;
    }

...
gboolean paddle_move(gpointer user_data,int p, int sense)
{
    ...
    GdkRectangle old = player.rect;

    // Works out the new position of the paddle.
    player.rect.y = CLAMP(game->player.rect.y - game->player.step.y, 0, y_max);

    // Redraws the paddle.
    redraw_item(game->ui.area, &old, &player.rect);

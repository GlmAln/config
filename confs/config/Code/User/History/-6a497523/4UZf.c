...
void set_play(Game* game)
{
    ...
    gtk_button_set_sentitive(game->ui->stop_button,FALSE);
...
void set_pause(Game* game)
{
    ...
    gtk_button_set_sentitive(game->ui->stop_button,TRUE);
    game->disc.event = g_source_remove(4, on_move_disc, game);
}
...
void on_start(GtkButton *button, gpointer user_data)
{
    ...
    if (game->disc.event == 0)
        game->disc.event = g_timeout_add(4, on_move_disc, user_data);
